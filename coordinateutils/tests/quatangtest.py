...
a2 = coordinateutils.quat(2,1,8,5)
b = coordinateutils.G3VectorQuat([a, a**2, 2*a, a2])
...
assert((np.asarray(coordinateutils.ang_to_quat(alpha, delta)) == np.asarray(coordinateutils.G3VectorQuat([coordinateutils.c_ang_to_quat(a[0], a[1]) for a in angles]))).all())
