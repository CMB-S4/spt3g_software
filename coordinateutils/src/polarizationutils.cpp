...
{
	/*
	 * Computes polarization rotation from Galactic to Equatorial
	 * coordinates.
	 */
    ...
    std::vector<double> &pol_in)
{
	...
	pol_angle_gal_to_eq_arr((double *) &l[0], (double *) &b[0],
	    (double *) &pol_in[0], (double *) &pol_out->front(), size);
...
pol_angle_gal_to_eq_py(T l, T b, T pol_in)
{
	...
	std::copy(bp::stl_input_iterator<double>(l),
	    bp::stl_input_iterator<double>(), d_l.begin());
	std::copy(bp::stl_input_iterator<double>(b),
	    bp::stl_input_iterator<double>(), d_b.begin());
	std::copy(bp::stl_input_iterator<double>(pol_in),
	    bp::stl_input_iterator<double>(), d_pol.begin());

#ifdef OPENMP_FOUND
	/*
	 * Boost Python lists are stupid and don't allow setting values at an
         * index. So, we use a temp array and then populate the list at the
	 * end.
	 */
...
{
	/*
	 * Computes polarization rotation from Equatorial to Galactic
	 * coordinates.
	 */

	/* Equatorial coordinates of Galactic pole */
	static const double gal_alpha = 192.85948098508993*M_PI/180.0;
	static const double gal_theta =
	    M_PI/2.0 - 27.128251257308968*M_PI/180.0;
...
pol_angle_eq_to_gal_py(T alpha, T delta, T pol_in)
{
	...
	for (i=0; i<size; i++)
		res_pol[i] =
		    pol_angle_eq_to_gal(d_alpha[i], d_delta[i], d_pol[i]);
...
#else
	for (i=0; i<size; i++)
		pol_out.append(pol_angle_eq_to_gal(d_alpha[i], d_delta[i],
		    d_pol[i]));
#endif
	return pol_out;
}
...
{
	/*
	 * Computes polarization rotation from between a reference coordinate
	 * system and a new coordinate system with pole at position
	 * (pole_theta, pole_phi) in the reference system. Phi is the
	 * azimuthal position, and Theta is the angular distance from the
	 * reference pole.
	 */
	...
	pol_angle_coord_sin_cos(pole_theta, pole_phi, pix_theta, pix_phi,
	    &rot_sin, &rot_cos);
    ...
    double pix_phi, double *rot_sin, double *rot_cos)
{
	...
	*rot_sin =
	    asin(sin(side_ref_new)*sin(angle_new_pixel)/sin(side_new_pixel));
	*rot_cos =
	    acos((cos(side_ref_new)-cos(side_ref_pixel)*cos(side_new_pixel))/
...
PYBINDINGS("coordinateutils"){
	bp::def("pol_angle_gal_to_eq", pol_angle_gal_to_eq,
	        ...
	        "Compute polarization rotation from Galactic to Equatorial "
	        "coordinates. Returns rotated polarization angle. (IAU)");
	...
	bp::def("pol_angle_eq_to_gal", pol_angle_eq_to_gal,
	        ...
	        "Compute polarization rotation from Equatorial to Galactic "
	        "coordinates. Returns rotated polarization angle. (IAU)");
	...
	bp::def("pol_angle_coord_transform", pol_angle_coord_transform,
	        (bp::arg("pole_theta"), bp::arg("pole_phi"),
	         bp::arg("pix_theta"), bp::arg("pix_phi"), bp::arg("pol")),
		"Rotate input IAU polarization angle to a coordinate system "
	        "defined in equatorial coordinates by a pole at (pole_theta, "
	        "pole_phi) from standard equatorial coordinates relative to "
	        "Earth.");
}
