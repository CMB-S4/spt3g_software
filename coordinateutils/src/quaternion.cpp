...
the general alpha delta angle to x-y-z mapping for one of the coordiate systems.
For the euclidean quaternion representation, the z coordinate = -sin(elevation).
and the z coordinate = sin(declination)
 **/

// Quaternion utilities

quat cross3(quat u, quat v) {
	/**
	   computes the euclidean cross product from the last three entries in the quaternion
	 **/
	return quat( 
	    0, 
...
double dot3(quat a, quat b) {
	/**
	   computes the euclidean dot product from the last three entries in the quaternion
	 **/
...
quat project_on_plane( quat plane_normal, quat point){
	/**
...
bool sloppy_eq( quat a, quat b, double slop = 1e-6){
	/**
...
quat get_transform_quat( double as_0, double ds_0,
	...
	//trans_asds_1 and aede_1 should now be the same up to a rotation around aede_0
	quat trans_asds_1 = tquat * asds_1 / tquat;

	//project them on to a plane and find the angle between the two vectors
	//using  ae_0, de_0 as the normal since we are rotating around that vector.
...
std::vector<double> test_trans(double az_0, double el_0,
				double ra_0, double dec_0,
				double az_1, double el_1,
				double ra_1, double dec_1,
...
std::vector<double> test_gal_trans(double az_0, double el_0,
				    double ra_0, double dec_0,
				    double az_1, double el_1,
				    double ra_1, double dec_1,
				    double az_t, double el_t){
...
double test_gal_trans_rot(double ra, double dec){
...
quat offsets_to_quat(double x_offset, double y_offset) {
	/**
	   Rotates the point (1,0,0) by the rotaiton matrix for the y_offset and then the 
	   rotation matrix for the x_offset
	 **/
...
quat get_origin_rotator(double alpha, double delta) {
	/**
	   rotates the point (1,0,0) to the point specified by alpha and delta via a rotation
	   about the y axis and then the z axis
	 **/
...
void print_fk5_j2000_to_gal_quat() {
	...
	std::cout << get_transform_quat(
		...
		0,-0.7853981633974483, 5.750520098164818, -1.2109809382060603) << std::endl;
}

quat get_fk5_j2000_to_gal_quat() {
	/**
	   returns the quaternion that rotates fk5j2000 to galacticj2000 coordinates
	 **/
	return quat(0.4889475076,-0.483210684,0.1962537583,0.699229742);
}

void create_det_az_el_trans(const G3Timestream & az, const G3Timestream & el, 
...
void create_lazy_det_ra_dec_trans(const G3Timestream & ra, const G3Timestream & dec, 
				  G3VectorQuat & trans_quats){
	/**
...
void create_det_ra_dec_trans(const G3Timestream & az_0, const G3Timestream & el_0, 
			     ...
			     G3VectorQuat & trans_quats){
	/**
	   computes the transform that takes (1,0,0) to the point (ra_0, dec_0) and properly
	   handles rotation about the (ra_0, dec_0) point with the inclusion of the second
	   set of points.
	...
	for (size_t i=0; i < ra_0.size(); i++){	
		trans_quats[i] = (get_transform_quat(az_0[i], -el_0[i], ra_0[i], dec_0[i],
						     az_1[i], -el_1[i], ra_1[i], dec_1[i])
				  * get_origin_rotator(az_0[i], -el_0[i]));
	}
}

void convert_ra_dec_trans_to_gal(const G3VectorQuat & radec_trans,
				 G3VectorQuat & gal_trans){
	/**
	   converts a rotation from (1,0,0) to fk5 j2000 into a rotation that takes
	   (1,0,0) to our galactic (l,b)
	 **/
...
void get_detector_pointing( 
	...
	/**
	   for a detector x/y offset and a boresight position specified by trans_quat
	   with a given coordinate system coord_sys
...
std::vector<double> convert_celestial_offsets_to_local_offsets(
	G3VectorQuat trans_vec, double x_offset_celest, double y_offset_celest){
